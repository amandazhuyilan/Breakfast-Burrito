...
# - When a player lands on other player's property, the amount of rent paid will be correct.
# - No trading properties between players or selling back to the bank allowed once the property is purchased.
...
# - Each turn is logged as "action", "price", "landed_on", "player", "roll" sequence.
# - Player only pays for the property rent if it is already purchased by another player.
...
def Monopoly():
    ...
    rent_money = []  # keeping track of money made from rent of each player
    # keeping track of the properties purchased and who owns it: 'property_name': 'owner_name'
    property_dict = {}


    if len(sys.argv) < 2:
        print "No input file name! Add file name and try again!\n"
        sys.exit(0)
    ...
    try:
        with open(sys.argv[1], 'r') as input_file:
            ...
            for index in range(len(content_lines)):
                ...
                for sub_line in line:
                    ...
                    # # add names into players_name and players_info
                    if entry[0] == "\"player\"":
                        player_name = entry[1]
            ...
            for index in range(len(content_lines)):
                ...
                # for example if there's 3 players, at the 4th turn the input line will correspond to the 4%3=1st player's data
        ...
        print "Monoploy text file invalid! Check file name and try again \n"
        sys.exit(0)
